...
def get_notice(request):
    if request.method == 'GET':
        ...
        new_notice = notice_collection.find(
            {'id_server_notice': {'$gt': 800}}).sort('id_server_notice', -1).limit(1)
        new_notice = list(new_notice)
        new_notice = new_notice[0]
        new_notice = new_notice['id_server_notice']

        check_student = student_collection.find(
        ...
        if check_student != []:
            ...
            student_collection.update_one(
                {'student_id': sid}, {'$set': {"last_server_id": last_notice}})
        else:
            new_student = {}
        ...
        if type == 0:  # tag별 + 최신 업데이트 100개
            id = notice_collection.find({"tag_num": tag}, {
                                        "_id": 0, "id_homepage_notice": 0}).sort("id_server_notice", -1).limit(100)
            result = list(id)[:100]
        if type == 1:  # tag별 + 최신 업데이트 100개 중 조회수 순
            id = notice_collection.find({"tag_num": tag}, {"_id": 0, "id_homepage_notice": 0}).sort(
                "id_server_notice", -1).limit(100).sort("watch", -1)
            result = list(id)[:100]
        if type == 2:  # tag 상관 없이 + 최신 업데이트 100개
            id = notice_collection.find({"tag_num": {'$lt': 10}}, {
                                        "_id": 0, "id_homepage_notice": 0}).sort("id_server_notice", -1).limit(100)
            result = list(id)[:100]
        if type == 3:  # tag 상관 없이 +최신 업데이트 100개 중 조회수 순
            id = notice_collection.find({"tag_num": {'$lt': 10}}, {"_id": 0, "id_homepage_notice": 0}).sort(
                "id_server_notice", -1).limit(100).sort("watch", -1)
            result = list(id)[:100]
...
def test_input(request):
    if request.method == 'GET':
        ...
        db = client.noticeDB
        notice_collection = db.notice
        data = {'title': title, 'summarize': summarize,
                "tag": tag, "writer": writer, "date": date, "link": link}
        print(data)
